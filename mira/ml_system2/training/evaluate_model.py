#!/usr/bin/env python3
"""
Evaluate LightGBM Model (ml_system2 - shared)

Works with any model trained by train_model.py. Reads feature_columns.json
from the model directory to know which features to use.

Usage:
    python3 evaluate_model.py \
        --model ./results/sketch_adv/lightgbm_model.txt \
...
def evaluate_model(model_path: str, test_csv: str):
    ...
    if len(available) < len(feature_cols):
        ...
        print(f"[WARNING] {len(missing)} features missing in test data: {list(missing)[:3]}...")

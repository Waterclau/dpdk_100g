#!/usr/bin/env python3
"""
Train LightGBM with Sketch-Only Features (14 features)

Uses only OctoSketch multi-scale + Ring Buffer temporal features.
...
def train_and_export(train_csv, val_csv, output_dir):
    """Train LightGBM with sketch features only"""

    print("=" * 70)
    print("SKETCH-ONLY MODEL - LightGBM Training")
    print(f"Features: {len(SKETCH_FEATURES)} (OctoSketch + Ring Buffer)")
    print("=" * 70)

    df_train = pd.read_csv(train_csv)
    df_val = pd.read_csv(val_csv)

    feature_cols = [c for c in get_feature_columns() if c in df_train.columns]
    missing = [c for c in SKETCH_FEATURES if c not in df_train.columns]
    if missing:
    ...
    for cls in label_encoder.classes_:
        ...
        print(f"  {cls:10s}: Train={train_count:4d}  Val={val_count:4d}")
    ...
    print(f"\n[TRAINING] {len(feature_cols)} sketch features, {num_boost_round} rounds...")
    model = lgb.train(
        params, train_data,
...
def main():
    parser = argparse.ArgumentParser(description='Train LightGBM with sketch features only')

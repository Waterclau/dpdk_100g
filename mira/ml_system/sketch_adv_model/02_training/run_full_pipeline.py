#!/usr/bin/env python3
"""
Full Sketch-ADV Training Pipeline

Runs the complete sketch-adv training:
  1. Trains LightGBM with 64 sketch-adv features
  2. Evaluates on the test set
  3. Runs model comparison (RF, HistGBM, MLP, KNN, SGD)
...
from feature_groups import SKETCH_FEATURES_ALL


def run_lightgbm(train_csv, val_csv, test_csv, output_dir):
    """Train and evaluate LightGBM with sketch-adv features"""
    group_dir = Path(output_dir) / 'sketch_adv' / 'lightgbm'

    print(f"\n{'='*70}")
    print(f"  LIGHTGBM - SKETCH-ADV ({len(SKETCH_FEATURES_ALL)} features)")
    print(f"{'='*70}")

    # Train
    train_script = str(Path(__file__).parent / 'train_sketch_adv.py')
    subprocess.run([
    ...
    eval_script = str(Path(__file__).parent / 'evaluate_sketch_adv.py')
    subprocess.run([
...
def run_model_compare(train_csv, val_csv, test_csv, output_dir):
    """Run multi-model comparison with sketch-adv features"""
    alt_dir = Path(output_dir) / 'sketch_adv' / 'alt_models'

    print(f"\n{'='*70}")
    print(f"  MODEL COMPARISON - SKETCH-ADV")
    print(f"{'='*70}")
...
def print_comparison_table(lgb_result, alt_result):
    ...
    print("\n")
    print("=" * 80)
    print("  SKETCH-ADV MODEL COMPARISON (64 features)")
    print("=" * 80)
    ...
    # LightGBM results
    if lgb_result:
        num_feat = lgb_result.get('num_features', len(SKETCH_FEATURES_ALL))
        rows.append({
            'model': 'LightGBM',
            'features': num_feat,
    ...
    if alt_result and 'models' in alt_result:
        num_feat = alt_result.get('num_features', len(SKETCH_FEATURES_ALL))
...
def main():
    parser = argparse.ArgumentParser(
        description='Run full sketch-adv training pipeline (64 features)'
    )
    ...
    print("=" * 70)
    print("  SKETCH-ADV TRAINING PIPELINE")
    print(f"  {len(SKETCH_FEATURES_ALL)} features (14 global + 48 per-protocol + 2 pkt size)")
    print("=" * 70)
    print(f"\nModels: LightGBM" + (" + RF, HistGBM, MLP, KNN, SGD" if not args.lightgbm_only else ""))
    ...
    combined = {
        ...
        'num_features': len(SKETCH_FEATURES_ALL),
        'lightgbm': lgb_result,
        'alt_models': alt_result,
    }
    with open(output_path / 'sketch_adv_results.json', 'w') as f:
        json.dump(combined, f, indent=2)

    # Save CSV summary
    if rows:
        df = pd.DataFrame(rows)
        csv_path = output_path / 'sketch_adv_summary.csv'
        df.to_csv(csv_path, index=False)
        print(f"\n[SAVED] Combined results: {output_path / 'sketch_adv_results.json'}")
        print(f"[SAVED] Summary CSV:      {csv_path}")

    print("\n[DONE] Sketch-ADV training pipeline complete!")

#!/usr/bin/env python3
"""
Prepare Dataset for Sketch-ADV Training (64 features)

Combines multiple CSV files, filters to sketch_adv features,
...
def main():
    parser = argparse.ArgumentParser(description='Prepare sketch-adv dataset (64 features)')
    ...
    # Filter to sketch_adv features (64)
    combined = filter_dataframe(combined, include_adv=True)

    expected = get_feature_columns(include_adv=True)
    available = [c for c in expected if c in combined.columns]
    print(f"[MODE] sketch_adv: {len(available)}/{len(expected)} features available")
    ...
    print(f"\n[DONE] Features: {len(available)} | Mode: sketch_adv")
